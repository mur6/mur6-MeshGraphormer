...
out_path = "./hand_repro_tsv/outputs"
...
def get_mano_coord(mesh_model, mano_param, cam_param):
    ...
    R, t = np.array(cam_param["R"], dtype=np.float32).reshape(3, 3), np.array(cam_param["t"], dtype=np.float32).reshape(
        3
    )
...
def extract(data_split):
    ...
    for aid in db.anns.keys():
        ...
        if hrnet_2dpose_output["image_id"] == image_id:
            ...
            if data_split == "train":
                cam_param, mano_param, joint_cam = (
                    data[db_idx]["cam_param"],
                    data[db_idx]["mano_param"],
                    data[db_idx]["joint_3d"],
                )
    ...
    for i in tqdm(range(len(datalist))):
        ...
        data = datalist[i]
        img_id = data["img_id"]
        img_path, img_shape, smpl_param, cam_param = (
            data["img_path"],
            data["img_shape"],
            data["mano_param"],
            data["cam_param"],
        ...
        if data_split == "train":
            error, img_data, gt_2d_joints, gt_3d_joints_norm = compute_3d_joints_error(
                mano_mesh_model, smpl_param, cam_param, gt_3d_joints, img_path
            )
            ...
            labels.append(
                {
                    "center": center,
                    "scale": scale,
                    "2d_joints": gt_2d_joints_tag.tolist(),
                    "has_2d_joints": 1,
                    "3d_joints": gt_3d_joints_tag.tolist(),
                    "has_3d_joints": 1,
                    "hrnet_2d_joints": hrnet_keypoint2d,
                    "has_hrnet_2d_joints": 1,
                    "pose": smpl_pose_camera_corrd,
                    "betas": smpl_shape_camera_corrd,
                    "has_smpl": 1,
                }
            )
        else:
            ...
            labels.append(
                {
                    ...
                    "hrnet_2d_joints": hrnet_keypoint2d,
                    "has_hrnet_2d_joints": 1,
                    "pose": smpl_pose_camera_corrd.tolist(),
                    "betas": smpl_shape_camera_corrd.tolist(),
                    "has_smpl": 1,
                }
            )
