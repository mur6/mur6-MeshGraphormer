...
def run(args, train_dataloader, dataset_type, Graphormer_model, mano_model, renderer, mesh_sampler):
    ...
    optimizer = torch.optim.Adam(
        params=list(Graphormer_model.parameters()), lr=args.lr, betas=(0.9, 0.999), weight_decay=0
    )
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        # forward-pass
        pred_camera, pred_3d_joints, pred_vertices_sub, pred_vertices = Graphormer_model(
        ...
        loss_reg_3d_joints = keypoint_3d_loss(
            criterion_keypoints, pred_3d_joints_from_mesh, gt_3d_joints_with_tag, has_3d_joints
        )
        ...
        if iteration % args.logging_steps == 0 or iteration == max_iter:
            ...
            logger.info(
                " ".join(["eta: {eta}", "epoch: {ep}", "iter: {iter}", "max mem : {memory:.0f}",]).format(
                    eta=eta_string, ep=epoch, iter=iteration, memory=torch.cuda.max_memory_allocated() / 1024.0 / 1024.0
                )
...
def run_aml_inference_hand_mesh(
    args, val_loader, Graphormer_model, criterion, criterion_vertices, epoch, mano_model, mesh_sampler, renderer, split
):
    ...
    with torch.no_grad():
        for i, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            # forward-pass
            pred_camera, pred_3d_joints, pred_vertices_sub, pred_vertices = Graphormer_model(
                images, mano_model, mesh_sampler
            )
...
def parse_args():
    ...
    parser.add_argument("--joints_3d_loss_weight", default=15.0, type=float)
    parser.add_argument("--vertices_loss_weight", default=5.0, type=float)
    ...
    #########################################################
    parser.add_argument(
        "--num_hidden_layers", default=-1, type=int, required=False, help="Update model config if given"
    )
